...
30、FI 31、ENE 32、STOCHRSI 33-75、其他技术指标...
```
...
# 周末技术分析（30-50分钟）
scripts\technical_analysis.bat

# 完整分析（60-90分钟）
scripts\full_analysis.bat
```
...
为了简化日常操作，系统提供了以下便捷脚本（位于 `scripts/` 目录）：

**a. 快速更新脚本 - quick_update.bat**
```bash
# 仅更新基础数据（实时行情、资金流向等）
# 耗时：15-20分钟
# 适用：每个交易日收盘后
...
**b. 技术分析脚本 - technical_analysis.bat** ⭐ **新增**
```bash
# 仅运行技术分析和选股（不包含基础数据抓取）
# 前置条件：已运行过 quick_update.bat 和 historical_data_job.py
# 耗时：30-50分钟
# 适用：周末或需要技术分析时
scripts\technical_analysis.bat
```

**c. 完整分析脚本 - full_analysis.bat**
```bash
# 依次运行所有任务（包含基础数据抓取）
# 耗时：60-90分钟
# 适用：周末或节假日进行全面分析
...
**详细使用说明请参考：** [scripts/README.md](scripts/README.md) 和 [docs/INDEPENDENT_TASKS_GUIDE.md](docs/INDEPENDENT_TASKS_GUIDE.md)
...
# 抓取历史数据（支持批量）
python instock/job/historical_data_job.py 2024-01-01          # 单日
python instock/job/historical_data_job.py 2024-01-01 --test   # 测试模式（30只股票）
python instock/job/historical_data_job.py 2024-01-01 2024-01-31  # 日期范围
```
...
详细指南请参考：[docs/INDEPENDENT_TASKS_GUIDE.md](docs/INDEPENDENT_TASKS_GUIDE.md)
...
> **注意：** `run_web.bat` 位于 `instock/bin/` 目录下，需要先切换到该目录或使用完整路径执行。
