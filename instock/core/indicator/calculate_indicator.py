...
def get_indicators(data, end_date=None, threshold=120, calc_threshold=None):
    try:
        ...
        logging.debug(f"开始处理数据，原始形状: {data.shape}")
        
        # 如果指定了截止日期，过滤数据
        if end_date is not None:
            # 【关键修复】统一日期类型，避免比较错误
            # data['date'] 可能是 datetime.date 或 str，需要统一处理
            if isinstance(end_date, str):
                # 如果 end_date 是字符串，确保 data['date'] 也是字符串
                if hasattr(data['date'].iloc[0], 'strftime'):
                    # data['date'] 是 datetime 类型，转换为字符串
                    data['date'] = data['date'].apply(lambda x: x.strftime('%Y-%m-%d') if hasattr(x, 'strftime') else str(x))
                # 现在可以直接比较字符串
                data = data[data['date'] <= end_date].copy()
            else:
                # end_date 是 datetime.date 类型
                if isinstance(data['date'].iloc[0], str):
                    # data['date'] 是字符串，转换为 datetime.date
                    data['date'] = pd.to_datetime(data['date']).dt.date
                # 现在可以比较 datetime.date
                data = data[data['date'] <= end_date].copy()
            logging.debug(f"日期过滤后形状: {data.shape}")
        
        # 如果指定了计算阈值，只用最后N天数据
        if calc_threshold is not None:
            data = data.tail(n=calc_threshold).copy()
            logging.debug(f"阈值过滤后形状: {data.shape}")
        
        # 【关键修复】将所有数值列转换为 float64，避免 TA-Lib 类型错误
        numeric_columns = ['open', 'close', 'high', 'low', 'volume', 'amount', 'p_change', '振幅', '涨跌额', '换手率']
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            # 【关键修复】再次创建深拷贝，确保在 np.errstate 上下文内数据是可写的
            data = data.copy(deep=True)
            ...
            data['macd'] = pd.Series(macd_result[0], index=data.index)
            data['macds'] = pd.Series(macd_result[1], index=data.index)
            data['macdh'] = pd.Series(macd_result[2], index=data.index)
            # 【修复】使用 fillna() 而不是直接修改 .values
            ...
            data['kdjk'] = pd.Series(kdjk_vals, index=data.index)
            data['kdjd'] = pd.Series(kdjd_vals, index=data.index)
            # 处理NaN
            ...
            - nbdevdn=2：下轨标准差倍数
            """
            boll_ub_vals, boll_vals, boll_lb_vals = tl.BBANDS(
                data['close'].values,  # 收盘价
                timeperiod=20,  # 周期
                nbdevup=2,  # 上轨标准差
                nbdevdn=2,  # 下轨标准差
            ...
            data['boll_ub'] = pd.Series(boll_ub_vals, index=data.index)
            data['boll'] = pd.Series(boll_vals, index=data.index)
            data['boll_lb'] = pd.Series(boll_lb_vals, index=data.index)
            # 处理NaN
            ...
            4. 乘以100转换为百分比
            """
            # 计算均价（注意：volume单位是手，需要乘以100转换为股）
            data.loc[:, 'm_price'] = data['amount'].values / (data['volume'].values * 100)
            # 前一日均价（shift函数向前移动1位）
            ...
            # 处理异常值
            data['cr'] = data['cr'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data.loc[:, 'avs'] = tl.SUM(data['av'].values.astype(float), timeperiod=26)
            # 下跌日成交量（涨跌幅<0）
            data.loc[:, 'bv'] = np.where(data['p_change'].values < 0, data['volume'].values, 0)
            data.loc[:, 'bvs'] = tl.SUM(data['bv'].values.astype(float), timeperiod=26)
            # 平盘日成交量（涨跌幅=0）
            data.loc[:, 'cv'] = np.where(data['p_change'].values == 0, data['volume'].values, 0)
            data.loc[:, 'cvs'] = tl.SUM(data['cv'].values.astype(float), timeperiod=26)
            # VR公式
            data.loc[:, 'vr'] = (data['avs'].values + data['cvs'].values / 2) / (data['bvs'].values + data['cvs'].values / 2)
            # 处理异常值
            data['vr'] = data['vr'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['pdi'] = data['pdi'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['mdi'] = data['mdi'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['dx'] = data['dx'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['vwma'] = data['vwma'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['stochrsi_k'] = data['stochrsi_k'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['esa_ci'] = data['esa_ci'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['ar'] = data['ar'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['br'] = data['br'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
            ...
            data['rvi'] = data['rvi'].fillna(0.0).replace([np.inf, -np.inf], 0.0)
        ...
        # 安全地获取股票代码，避免访问只读DataFrame
        try:
            if data is not None and 'code' in data.columns:
                code_val = data['code'].iloc[0] if len(data) > 0 else '未知'
                logging.error(f"股票 代码: {code_val}")
            else:
                logging.error("无法获取股票代码")
        except Exception:
            pass
    return None
...
def get_indicator(code_name, data, stock_column, date=None, calc_threshold=90):
    code = None  # 初始化 code 变量
    try:
        # 【关键修复】立即创建深拷贝，避免多线程环境下的只读问题
        data = data.copy(deep=True)
