...
功能说明：
本模块定义了所有技术指标在Web端的展示配置，包括：
1. 指标名称和标题
2. 指标字段列表
3. 指标详细说明（含外部链接）

核心数据结构：
indicators_dic - 指标配置列表
每个元素是一个字典，包含：
- title：指标简称（用于Tab标签）
- desc：指标详细说明（HTML格式，含学习链接）
- dic：指标字段元组（用于图表绘制）
...
使用场景：
- visualization.py中遍历此列表创建指标Tab页
- 每个指标一个Tab，点击切换查看
- Tab标题显示title，下方显示desc详细说明
- 图表中绘制dic指定的字段

数据流程：
1. visualization.py导入indicators_dic
2. 遍历列表，为每个指标创建Tab
3. 根据dic中的字段名从数据源读取数据
4. 绘制折线图或柱状图
5. 在Tab下方显示desc HTML说明

字段命名规则：
- 单一指标：直接使用指标名（如"macd", "rsi"）
- 多线条指标：加后缀区分
  * macds：MACD信号线（signal）
  * macdh：MACD柱状图（histogram）
  * kdjk, kdjd, kdjj：KDJ的K、D、J三条线
  * boll_ub, boll, boll_lb：布林线上轨、中轨、下轨
  * cr-ma1, cr-ma2, cr-ma3：CR的三条均线

外部链接：
所有指标都提供了学习链接（https://www.ljjyy.com）
用户可以点击链接查看指标的：
- 计算公式
- 使用方法
- 买卖信号
...
配置扩展：
如需添加新指标：
1. 在calculate_indicator.py中实现计算逻辑
2. 在此文件中添加配置项
3. 确保字段名与计算结果一致
4. 提供详细的学习链接

依赖关系：
- 被visualization.py导入和使用
- 依赖tablestructure.py的get_field_cn()获取中文名称
- 依赖calculate_indicator.py计算的指标数据

注意事项：
1. dic中的字段名必须与DataFrame列名完全一致
2. desc是HTML格式，可以包含链接和样式
3. 指标顺序会影响Tab显示顺序
4. 建议按重要性或类型分组排列
5. 外部链接需要定期验证有效性
"""
