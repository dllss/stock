...
def save_nph_stock_selection_data(date, before=True):
    """
    综合选股数据任务
    已重构为调用独立任务文件: data_tasks/cn_stock_selection_job.py
    """
    if before:
        return
    
    # 导入并调用独立任务
    from instock.job.data_tasks import cn_stock_selection_job
    cn_stock_selection_job.save_selection_data(date, before=False)
