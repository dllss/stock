...
运行方式：
    python instock/job/lhb/lhb_detail_job.py
...
def save_lhb_detail_data(date, before=True):
    """
    主函数：抓取并保存龙虎榜明细数据
    ...
    try:
        ...
        # 步骤2: 抓取数据
        logging.info("=" * 20)
        logging.info(f"[{date}] 开始获取龙虎榜明细数据...")
        
        data = stf.fetch_stock_lhb_data(date)
        if data is None or len(data.index) == 0:
            logging.info("龙虎榜明细数据为空，跳过")
            return

        # 准备插入数据
        logging.info(f"💾 准备插入数据到表: {table_name} (龙虎榜明细)")
        ...
        if insert_result:
            logging.info(f"✅ 保存龙虎榜明细数据成功：{len(data)}条")
            logging.info(f"   表名: {table_name}")
            logging.info(f"   主键: date, code")
            logging.info(f"   字段数: {len(data.columns)}")
        else:
            logging.error(f"❌ 保存龙虎榜明细数据失败！请检查上方错误信息")
            
    except Exception as e:
        logging.error(f"❌ 龙虎榜明细任务处理异常：{e}", exc_info=True)


def main():
    """主入口函数"""
    logging.info("=" * 20)
    logging.info("开始执行龙虎榜明细数据任务")
    logging.info("=" * 20)
    
    # 设置 before=False（盘后执行）
    runt.run_with_args(save_lhb_detail_data, False)
    
    logging.info("")
    logging.info("=" * 20)
    logging.info("龙虎榜明细数据任务执行完成")
    logging.info("=" * 20)
