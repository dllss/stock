...
行业板块资金流向数据任务 - 独立执行脚本
========================================
功能：抓取并保存行业板块资金流向数据（今日/3日/5日）
目标表：cn_stock_fund_flow_industry
API请求数：约30-50次（3个周期 × 页数）
执行时间：约3-5分钟

运行方式：
    python instock/job/fund_flow/industry_fund_flow_job.py
...
def run_check_industry_fund_flow(times):
    """
    顺序抓取行业板块资金流向数据
    ...
    try:
        ...
        for k in times:
            ...
            logging.info("")
            logging.info(f"[INFO] 开始获取{period_name}行业板块资金流向数据...")
            
            # 打印目标数据库表名
            tbs_table = tbs.TABLE_CN_STOCK_FUND_FLOW_INDUSTRY
            table_name = tbs_table['name']
            table_cn = tbs_table.get('cn', '行业资金流向')
            logging.info(f"   目标表: {table_name} ({table_cn})")
            
            _data_ = stf.fetch_stocks_sector_fund_flow(0, k)
        ...
        logging.error(f"❌ 行业板块资金流向任务异常：{e}", exc_info=True)
...
def save_industry_fund_flow_data(date, before=True):
    """
    主函数：抓取并保存行业板块资金流向数据
    ...
    try:
        ...
        results = run_check_industry_fund_flow(times)
        if results is None:
        ...
        logging.error(f"❌ 行业资金流向任务处理异常：{e}", exc_info=True)


def main():
    """主入口函数"""
    logging.info("=" * 20)
    logging.info("开始执行行业板块资金流向数据任务")
    logging.info("=" * 20)
    
    # 设置 before=False（盘后执行）
    runt.run_with_args(save_industry_fund_flow_data, False)
    
    logging.info("")
    logging.info("=" * 20)
    logging.info("行业板块资金流向数据任务执行完成")
    logging.info("=" * 20)
